...
public class Game extends javax.swing.JFrame {
    ...
    public Game(Jugador jugadores[]) {
        ...
        for (int i = 0; i < 16; i++) {
            cuadros.add(new Cuadro(paneles.get(i), comodin, lineasHorizontales.get(i), lineasHorizontales.get((i+4)), new Linea(), new Linea()));
        }
        int aux=0;
        for (int i = 0; i < 16; i++) { 
            if(i==4 || i==8 || i==12)aux++;
   ...
   //Método que cambia de color a todos las líneas y páneles relacionados con 
    ...
    public boolean finPartida(){
        ...
        for (int j = 0; j < cuadros.size()+1; j++) {
            ...
            if(j==cuadros.size()){
                ...
                //Itera por los jugadores
                for (int x = 0; x < 3; x++) {
                    //Consigue el mayor entre el jugador en x y el que está 
                    //frente a el
                    int mayor=Math.max(jugadores[x].getPuntaje(), jugadores[x+1].getPuntaje());
                    //Si el jugador en x es mayor a el jugador delante de el, lo
                    //asignará como ganador
                    if(mayor==jugadores[x].getPuntaje()) ganador=jugadores[x];
                    //Caso contrario asignará al jugador frente a el como 
                    //ganador
                    if(mayor==jugadores[x+1].getPuntaje()) ganador=jugadores[x+1];
                }
    ...
    /*
    //Este método calcula la línea superior cada cuadro por su posición
    public Linea superiorPC(Cuadro cuadro){
        int x = cuadro.getCoordenadax();
        int y = cuadro.getCoordenaday();
        return lineas.get();
    }
    //Este método calcula la línea inferior cada cuadro por su posición
    public Linea inferiorPC(Cuadro cuadro){
        int x = cuadro.getCoordenadax();
        int y = cuadro.getCoordenaday();
        return lineas.get();
    }
    //Este método calcula la línea izquierda cada cuadro por su posición
    public Linea izquierdaPC(Cuadro cuadro){
        int x = cuadro.getCoordenadax();
        int y = cuadro.getCoordenaday();
        return lineas.get();
    }
    //Este método calcula la línea derecha cada cuadro por su posición
    public Linea derechaPC(Cuadro cuadro){
        int x = cuadro.getCoordenadax();
        int y = cuadro.getCoordenaday();
        return lineas.get();
    }
    
    //Este método crea un cuadro con sus líneas respectivas a su posición y con
    //los atributos del cuadro entregado en parámetros
    private Cuadro cuadroCompleto(Cuadro cuadro){
        Cuadro cuadrito=new Cuadro(cuadro.getPanel(), null, superiorPC(cuadro),
                inferiorPC(cuadro), izquierdaPC(cuadro), derechaPC(cuadro), 
                cuadro.getCoordenadax(), cuadro.getCoordenaday());
